...
namespace IPOApi.Controllers
{
    public class UserManagementController : Controller
    {
        ...
        private readonly UserManagementData objData;
        public UserManagementController(IConfiguration configuration)
        {
            _configuration = configuration;
            objData = new UserManagementData(_configuration);
        }
        string constring = "";
        ...
        public IActionResult UsersList()
        {
            ...
            try
            {
                DataSet response = objData.getuserlist(constring); // Call to DataAccess
        ...
        [HttpPost("InsUser")]
        public IActionResult InsUser([FromBody] UserManagementnewModel insObj)
        {
            ...
            try
            {
                response = objData.InsUser(insObj, constring);
        ...
        [HttpPost("UpdUser")]
        public IActionResult UpdUser([FromBody] UserManagementnewModel insObj, string constring)
        {
            constring = _configuration.GetSection("Appsettings")["ConnectionStrings"].ToString();           
            //headerValue header_value = new headerValue();
            DataSet response = new DataSet();
            try
            {
                response = objData.UpdUsers(insObj, constring);
                var serializedProduct = JsonConvert.SerializeObject(response, Formatting.None);
                return Ok(serializedProduct);
            }
            catch (Exception e)
